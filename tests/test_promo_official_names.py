...
def test_playable_promo_uses_official_main_set_name_and_keeps_holo(tmp_path):
    ...
    with CatalogueRepository(database) as c:
        ...
        c.apply_name_mappings([
            EnglishNameMapping('DBT01','001','RRR','Playable Card','official-english','https://en/main',status='official'),
            EnglishNameMapping('DBT01','002','C','Energy','official-english','https://en/energy',status='official'),
        ])
    ...
    with CatalogueRepository(database) as c:
        ...
        assert card.source == 'official-name-match-provisional'
...
def test_official_and_fandom_name_disagreement_stays_unmapped(tmp_path):
    with CatalogueRepository(tmp_path / 'catalogue.sqlite3') as c:
        ...
        c.apply_name_mappings([
            EnglishNameMapping('DBT01','001','RRR','One Name','official-english','https://en/1'),
