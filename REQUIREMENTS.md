...
| Aggregate card prints / selected-print sorting | Implemented | The far-right filter-bar action is labelled `Aggregate card prints` and has a hover explanation that it compares every verified Japanese printing of the card, including a one-printing card. The selected-print offer panel has its own sort icon with Lowest price and Highest price choices. It reorders the already retrieved offers instantly: in-stock rows remain first, then sold-out and unknown-stock rows; each group sorts by price in the selected direction and unavailable prices stay last. |
