"""C-labo connector using its public exact serial search."""
...
class CLaboConnector(StoreConnector):
    """Read exact Japanese Vanguard prints from C-labo's public product list."""
    ...
    async def _get(self, url: str, *, params: dict[str, str]) -> httpx.Response:
        ...
        if response.status_code in (403, 429):
            raise StoreUnavailableError("C-labo did not permit this price request.")
        if response.status_code == 404:
            raise StoreUnavailableError("C-labo no longer exposes its product search page.")
    ...
    def parse_html(cls, card: CardPrint, page_content: str) -> list[StoreOffer]:
        ...
        seen: set[str] = set()
        for item in soup.select(".list_item, .list_item_cell"):
