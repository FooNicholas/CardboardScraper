"""TCG NOAH connector using its public exact serial search."""
...
class NoahConnector(StoreConnector):
    """Read exact Japanese Vanguard prints from TCG NOAH's public product list."""

    store_id = "noah"
    store_name = "TCG NOAH"
    base_url = "https://www.tcgshop-noah.net"
    search_url = f"{base_url}/product-list/248/0/normal"
    ...
    async def _get(self, url: str, *, params: dict[str, str]) -> httpx.Response:
        ...
        if response.status_code in (403, 429):
            raise StoreUnavailableError("TCG NOAH did not permit this price request.")
        if response.status_code == 404:
            raise StoreUnavailableError("TCG NOAH no longer exposes its product search page.")
