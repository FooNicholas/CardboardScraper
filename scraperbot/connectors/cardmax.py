"""Card Max connector using its public exact serial search."""
...
class CardMaxConnector(StoreConnector):
    """Read exact Japanese Vanguard prints from Card Max's public storefront."""

    store_id = "cardmax"
    store_name = "Card Max"
    base_url = "https://www.cardmax.jp"
    ...
    async def search(self, card: CardPrint) -> list[StoreOffer]:
        response = await self._request("post", self.search_url, data={"search": retailer_print_reference(card)})
        offers = self.parse_html(card, self._decode(response))
        for index, offer in enumerate(offers[:8]):
            if not offer.listing_url:
                continue
            try:
                detail = await self._request("get", offer.listing_url)
            except (httpx.HTTPError, StoreUnavailableError):
                continue
            offers[index] = self._with_detail_stock(offer, self._decode(detail))
        return offers

    async def _request(self, method: str, url: str, **kwargs: object) -> httpx.Response:
        headers = {"User-Agent": "JP-Price-Checker/0.1 (+approved personal price comparison)"}
        if self.client:
            response = await getattr(self.client, method)(url, headers=headers, **kwargs)
        else:
            async with httpx.AsyncClient(timeout=20, follow_redirects=True) as client:
                response = await getattr(client, method)(url, headers=headers, **kwargs)
        if response.status_code in (403, 429):
            raise StoreUnavailableError("Card Max did not permit this price request.")
        if response.status_code == 404:
            raise StoreUnavailableError("Card Max no longer exposes this card page.")
    ...
    def parse_html(cls, card: CardPrint, page_content: str) -> list[StoreOffer]:
        ...
        for item in soup.select("tr"):
            product_link = item.select_one('a[href*="shopdetail.html"]')
            if not product_link:
                continue
    ...
    def _with_detail_stock(cls, offer: StoreOffer, page_content: str) -> StoreOffer:
        ...
        availability = (
            ...
            if stock_count is not None or bool(soup.select_one(".add_cart, [name='BASKET']"))
