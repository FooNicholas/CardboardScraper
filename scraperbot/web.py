...
function renderFilters() { clear(filters); const rarities=[...new Set(searchCards.map(card=>card.rarity).filter(Boolean))].sort(), finishes=[...new Set(searchCards.map(card=>card.finish).filter(finish=>finish && finish!=='unknown'))].sort(), aggregate=aggregateCandidate(); if (!rarities.length && !finishes.length && !aggregate) { filters.hidden=true; return; } filters.hidden=false; if (rarities.length) { const group=document.createElement('div'); group.className='filter-group'; group.append(text('span','Rarity','filter-label')); for (const rarity of rarities) group.append(filterButton(rarity,activeRarities.has(rarity),()=>{ activeRarities.has(rarity) ? activeRarities.delete(rarity) : activeRarities.add(rarity); renderFilters(); renderCards(); })); filters.append(group); } if (finishes.length) { const group=document.createElement('div'); group.className='filter-group'; group.append(text('span','Finish','filter-label')); for (const finish of finishes) { const label=finish==='holo' ? 'Holo' : finish==='standard' ? 'Standard' : finish; group.append(filterButton(label,activeFinishes.has(finish),()=>{ activeFinishes.has(finish) ? activeFinishes.delete(finish) : activeFinishes.add(finish); renderFilters(); renderCards(); })); } filters.append(group); } if (activeRarities.size || activeFinishes.size) { const reset=document.createElement('button'); reset.type='button'; reset.className='filter-clear'; reset.textContent='Clear filters'; reset.addEventListener('click',()=>{ activeRarities.clear(); activeFinishes.clear(); renderFilters(); renderCards(); }); filters.append(reset); } if (aggregate) { const button=document.createElement('button'); button.type='button'; button.className='filter-button aggregate-button'; button.textContent='Sort by lowest'; button.title='Sort listed offers by lowest price across '+aggregate.family_print_count+' verified printing'+(aggregate.family_print_count===1?'':'s'); button.addEventListener('click',()=>compareFamily(aggregate.id)); filters.append(button); } }
...
async function compareFamily(id, refresh=false) { selectedId=id; highlightSelection(); setStatus('Sorting listings by lowest price…'); comparison.replaceChildren(text('p','Comparing listings across verified printings…','notice')); try { const data=await readJson(await fetch('/api/compare-family?id='+encodeURIComponent(id)+(refresh?'&refresh=1':''))); renderFamilyComparison(data); setStatus(''); } catch (error) { clear(comparison); setStatus(error.message,true); } }
