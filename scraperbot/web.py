...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .sort-icon { min-width:34px; font-size:1rem !important; line-height:1; }
    .sort-menu { position:absolute; z-index:2; right:0; top:calc(100% + 6px); display:grid; gap:4px; min-width:132px; padding:6px; background:var(--surface); border:1px solid var(--line); border-radius:10px; box-shadow:0 10px 24px #5e433a20; }
    .sort-menu button { width:100%; margin:0; text-align:left; white-space:nowrap; }
...
let selectedId = null, searchCards = [], activeRarities = new Set(), activeFinishes = new Set(), currentComparison = null, offerSort = 'lowest', sortMenuOpen = false;
...
async function search(raw) { const value=(raw || query.value).trim(); if (!value) { setStatus('Enter a card name to search.', true); return; } query.value=value; clear(results); clear(comparison); clear(filters); filters.hidden=true; selectedId=null; searchCards=[]; currentComparison=null; sortMenuOpen=false; activeRarities.clear(); activeFinishes.clear(); setStatus('Finding matching prints…'); searchButton.disabled=true; try { const data=await readJson(await fetch('/api/search?q='+encodeURIComponent(value))); if (!data.cards.length) { setStatus('No Japanese-market print matched that name. Try a shorter spelling.'); return; } searchCards=data.cards; renderFilters(); renderCards(); } catch (error) { setStatus(error.message,true); } finally { searchButton.disabled=false; } }
