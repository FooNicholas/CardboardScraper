...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } body { margin:0; min-height:100vh; background:radial-gradient(circle at 12% 0%, #30225a, transparent 37rem), #0d0e17; color:var(--ink); font:16px/1.5 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
    ...
    @media (max-width:600px) { main { padding-top:42px } form { padding:7px } button { padding:11px 13px } .card { align-items:flex-start } .card .code { white-space:normal } .offer { grid-template-columns:1fr auto; } .tag { grid-column:1 / -1; text-align:left; } }
  </style>
</head>
<body><main>
  ...
  <div id="status" aria-live="polite"></div><section id="results" class="result-list"></section><section id="comparison"></section>
</main><script>
