...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } body { margin:0; min-height:100vh; background:radial-gradient(circle at 12% 0%, #30225a, transparent 37rem), #0d0e17; color:var(--ink); font:16px/1.5 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
    ...
    .hint { color:var(--muted); font-size:.88rem; margin:12px 0 30px; } .hint button { color:var(--muted); background:transparent; border:1px solid var(--line); padding:4px 9px; margin-left:5px; font-size:.8rem; }
    ...
    @media (max-width:780px) { main { padding-top:42px } .workspace { grid-template-columns:1fr; gap:18px; } .search-panel { position:static; } form { padding:7px } button { padding:11px 13px } .card { align-items:flex-start } .card .code { white-space:normal } .offer { grid-template-columns:1fr auto; } .tag { grid-column:1 / -1; text-align:left; } }
  </style>
</head>
<body><main>
  ...
  <div class="workspace"><aside class="search-panel"><form id="search-form"><input id="query" type="search" maxlength="120" autocomplete="off" placeholder="Try: Youthberk, Haughty Peerage FFR" autofocus><button id="search-button">Search</button></form>
  ...
  <div id="status" aria-live="polite"></div></aside><div class="results-panel"><section id="results" class="result-list"></section><section id="comparison"></section></div></div>
</main><script>
...
async function search(raw) { const value=(raw || query.value).trim(); if (!value) { setStatus('Enter a card name to search.', true); return; } query.value=value; clear(results); clear(comparison); selectedId=null; setStatus('Finding matching prints…'); searchButton.disabled=true; try { const data=await readJson(await fetch('/api/search?q='+encodeURIComponent(value))); if (!data.cards.length) { setStatus('No local card print matched that name. Try a shorter spelling.'); return; } setStatus(data.cards.length+' matching print'+(data.cards.length===1?'':'s')+' — choose one to compare stores.'); for (const card of data.cards) { const button=document.createElement('button'); button.type='button'; button.className='card'; const copy=document.createElement('div'); copy.append(text('h2',card.english_name), text('p',card.japanese_name || 'Official English print')); button.append(copy,text('div',card.display_code,'code')); button.addEventListener('click',()=>compare(card.id)); results.append(button); } } catch (error) { setStatus(error.message,true); } finally { searchButton.disabled=false; } }
...
function renderComparison(data) { clear(comparison); const head=document.createElement('div'); head.className='comparison-head'; const copy=document.createElement('div'); copy.append(text('h2',data.card.english_name),text('p',(data.card.japanese_name ? data.card.japanese_name+' · ' : '')+data.card.display_code)); const refresh=document.createElement('button'); refresh.textContent='Refresh prices'; refresh.addEventListener('click',()=>compare(data.card.id,true)); head.append(copy,refresh); comparison.append(head); if (!data.offers.length) comparison.append(text('p','No matching store listing is available for this exact print right now.','notice')); for (const offer of data.offers) { const row=document.createElement('div'); row.className='offer'; const store=document.createElement(offer.listing_url ? 'a' : 'div'); store.textContent=offer.store_name; if (offer.listing_url) { const href=safeLink(offer.listing_url); if (href) { store.href=href; store.target='_blank'; store.rel='noopener noreferrer'; } } const detail=text('small',offer.raw_name); const storeWrap=document.createElement('div'); storeWrap.append(store,detail); row.append(storeWrap,text('div',offer.price_display,offer.availability==='in_stock'?'price':'sold'),text('div',offer.match_confidence.replaceAll('_',' '),'tag')); comparison.append(row); } const notices=[]; if (data.unavailable_stores.length) notices.push('Not listed: '+data.unavailable_stores.join(', ')); if (data.failed_stores.length) notices.push('Could not check: '+data.failed_stores.join(', ')); if (notices.length) comparison.append(text('p',notices.join(' · '),'notice')); }
async function compare(id, refresh=false) { selectedId=id; setStatus('Checking stores…'); comparison.replaceChildren(text('p','Comparing exact-print listings…','notice')); try { const data=await readJson(await fetch('/api/compare?id='+encodeURIComponent(id)+(refresh?'&refresh=1':''))); renderComparison(data); setStatus(''); } catch (error) { clear(comparison); setStatus(error.message,true); } }
