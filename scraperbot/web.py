...
function aggregateCandidate() { const cards=filteredCards(); if (!cards.length || cards[0].family_print_count<2 || !cards[0].japanese_name) return null; const first=cards[0]; return cards.every(card=>card.japanese_name===first.japanese_name && card.english_name===first.english_name) ? first : null; }
