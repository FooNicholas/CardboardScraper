...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .sort-icon { min-width:42px; font-size:1rem !important; line-height:1; letter-spacing:-.14em; }
...
let selectedId = null, searchCards = [], activeRarities = new Set(), activeFinishes = new Set(), currentComparison = null, comparisonMode = null, offerSort = 'lowest', sortMenuOpen = false;
...
async function search(raw) { const value=(raw || query.value).trim(); if (!value) { setStatus('Enter a card name to search.', true); return; } query.value=value; clear(results); clear(comparison); clear(filters); filters.hidden=true; selectedId=null; searchCards=[]; currentComparison=null; comparisonMode=null; sortMenuOpen=false; activeRarities.clear(); activeFinishes.clear(); setStatus('Finding matching prints…'); searchButton.disabled=true; try { const data=await readJson(await fetch('/api/search?q='+encodeURIComponent(value))); if (!data.cards.length) { setStatus('No Japanese-market print matched that name. Try a shorter spelling.'); return; } searchCards=data.cards; renderFilters(); renderCards(); } catch (error) { setStatus(error.message,true); } finally { searchButton.disabled=false; } }
...
function offerSortControl() { const control=document.createElement('div'); control.className='sort-control'; const icon=document.createElement('button'); icon.type='button'; icon.className='sort-icon'; icon.textContent=offerSort==='highest' ? '☷↓' : '☷↑'; icon.title='Sort displayed offers: '+(offerSort==='highest' ? 'Highest price' : 'Lowest price'); icon.setAttribute('aria-label',icon.title); icon.setAttribute('aria-expanded',String(sortMenuOpen)); icon.addEventListener('click',()=>{ sortMenuOpen=!sortMenuOpen; renderCurrentComparison(); }); control.append(icon); if (sortMenuOpen) { const menu=document.createElement('div'); menu.className='sort-menu'; menu.setAttribute('role','menu'); for (const [value,label] of [['lowest','Lowest price'],['highest','Highest price']]) { const choice=filterButton(label,offerSort===value,()=>{ offerSort=value; sortMenuOpen=false; renderCurrentComparison(); }); choice.setAttribute('role','menuitemradio'); choice.setAttribute('aria-checked',String(offerSort===value)); menu.append(choice); } control.append(menu); } return control; }
...
async function compareFamily(id, refresh=false) { selectedId=id; currentComparison=null; comparisonMode=null; sortMenuOpen=false; highlightSelection(); setStatus('Aggregating listings across card prints…'); comparison.replaceChildren(text('p','Comparing listings across verified printings…','notice')); try { const data=await readJson(await fetch('/api/compare-family?id='+encodeURIComponent(id)+(refresh?'&refresh=1':''))); renderFamilyComparison(data); setStatus(''); } catch (error) { clear(comparison); setStatus(error.message,true); } }
